...
class GPUModelRunner(
    ...
    def sample_tokens(
        ...
        active_sample_indices = np.nonzero(
            ~self.discard_request_mask.np[: self.input_batch.num_reqs]
        )[0].tolist()
        try:
            from vllm.v1.sample.logits_processor.paired_logits import (
                set_paired_logits_active_sample_indices,
            )
        except Exception:
            set_paired_logits_active_sample_indices = None

        if set_paired_logits_active_sample_indices is not None:
            set_paired_logits_active_sample_indices(active_sample_indices)
        try:
